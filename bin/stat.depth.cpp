...
int main(int argc,char* argv[]){
	if(argc!=5){
		cerr<<argv[0]<<"  <bed_file>  <samtools_depth_file>  <final_stat_file>  <stat_output>\n";
		exit(0);
	}
	...
	bam_file.erase(bam_file.end()-13,bam_file.end());
...
//	string bedcov_cmd=samtools_path+" index "+bam_file+";"+samtools_path+" bedcov "+smn_bed+" "+bam_file+" >"+bam_file+".smn.bedcov";
	string bedcov_cmd=samtools_path+" bedcov "+smn_bed+" "+bam_file+" >"+bam_file+".smn.bedcov";
	if(system(bedcov_cmd.c_str())==-1){
		cerr<<"Error:samtools bedcov error"<<endl;
		exit(1);
	}
	string final_stat_file(argv[3]);
	...
	if(system(awk_cmd.c_str())==-1){
		cerr<<"Error:awk error"<<endl;
		exit(1);
	};
	//cout<<awk_cmd<<endl;
	...
	//cout<<tmp_cmd<<endl;
	if(system(tmp_cmd.c_str())==-1){
		cerr<<"Error:paste error"<<endl;
		exit(1);
	}
	string rm_string="rm "+bam_file+".smn.bedcov "+bam_file+".smn.bedcov2";
	if(system(rm_string.c_str())==-1){
