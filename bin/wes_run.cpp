...
#include "gc.h"
using namespace::std;
void run_wes_container(string fid,cmd_para &env_para){
	...
	if(fid=="bwa"){
		...
		cmd1=bwa_path+" mem -R '@RG\\tID:"+env_para.output_prefix+"\\tSM:"+env_para.output_prefix+"\\tPL:"+pl+"' -t "+mem_threads+" "+env_para.m_config2["ref_fasta"]+" "+whole_out+".trim.r1.fq.gz "+whole_out+".trim.r2.fq.gz | "+samtools_path+" view -b - | "+samtools_path+" sort -@ "+sort_threads+" -O bam -T "+env_para.output_dir+"/tmp123 - -o "+whole_out+".sort.bam";
